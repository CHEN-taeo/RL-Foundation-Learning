...
class ReplayBuffer():   # 定义数据存储工具库
    def __init__(self,max_size=10000): # 开店前的准备：买一个收集箱   # 代表收集箱的最大容量
        self.buffer=[]   # 存放所有的五元组（顾客反馈）
        self.position=0 # 用于记录当前存到了哪个位置，方便超过最大容量时覆盖最旧的数据
        self.max_size = max_size
    def push(self,s,a,r,s_next,done):
    # 往回放池存数据
        if len(self.buffer)<self.max_size:  # 分支1：回放池没有放满的情况下，多余的位置占上位置
            self.buffer.append(None)  # 多余的位置空出来，先空位
        self.buffer[self.position]=(s, a, r, s_next, done) # 存放一条经验五元组,后存经验
        self.position=(self.position+1) %self.max_size  # 经验组更新


    def sample(self,batch_size):
        batch=random.sample(self.buffer,batch_size)   # 随机且不重复的从数据库中采样
        s,a,r,s_next,done=zip(*batch)     # 拆分批量元组
        return s,a,r,s_next,done    # 返回五个变量
    def __len__(self):
        return len(self.buffer)    # 返回当前回放池中存了多少条数据
net = SimpleDQN()
print(net)   # 打印网络的结构布局，检查是否正确



#____________________________________测试代码______________________________________
